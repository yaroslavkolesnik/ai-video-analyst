...
**Поправка від 2026-09-14 (після ручного проходження гайда A).** Додано блок
`STARTING STATE`: модель зобов’язана першою подією віддати `ui_state` з тим, що
вже було на екрані до будь-якої дії — значення кожного читабельного фільтра і
видимий лічильник.

Причина знайдена не в коді, а при спробі пройти гайд руками. Гайд ніде не каже,
з якого стану починати, і читач із уже виставленими фільтрами отримає інші числа.
Перевірка таймлайну A показала, чому це не лікується на боці рендеру: **перша
подія там — на 11.5 с, і це вже сама дія.** Перші одинадцять секунд запису не
дали жодної події, тобто початкового стану в даних не існує взагалі.

Схема не змінюється: подія використовує наявний `kind: "ui_state"`, тож ні zod,
ні `OBSERVATION_RESPONSE_SCHEMA` не чіпались. Споживач — чиста функція
`findStartingState()` у `reduce.ts`: бере **видиму** `ui_state`, що передує
кожній `ui_action`, і повертає `null`, якщо такої немає. Записи, зроблені до цієї
поправки, лишаються валідними і рендерять чесне «starting state was not
recorded» замість вигаданого стану за замовчуванням.

`ui_state` не має ребра до нумерованого списку — ні до, ні після поправки. Це
закріплено тестом «never becomes a step, however early it sits».
